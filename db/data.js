...
const fetchStatusData = (id, status_id) => {
  ...
  const html = `
    <div class="card-header">
                ...
                <div class="card-body">
                    <ul class="list-group list-group-flush sortable ui-sortable" data-status-id="${
                      currStatus.id
                    }" id="sort${currStatus.id}">
                   
                    </ul>
                </div>`;
  $(id).append(html);

  currStatusData.forEach(function(item){
    // console.log(item);
    
   return $(`${id} ul#sort${status_id}`).append(`<li class="list-group-item ui-sortable-handle" data-task-id="${item.id}" style="z-index:99; display:block;">${item.title}</li>`)
  })
  //   console.log(html);
};
{
